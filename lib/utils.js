...
/**
 * Generate sequential slots based on booked appointments and durations
 */
export function generateDynamicSlots(startTime = '09:00', endTime = '17:00', defaultDuration = 30, appointments = []) {
  const slots = [];
  ...
  while (current < end) {
    // Find if there is an appointment starting at or overlapping the current minutes
    const activeAppt = sortedAppts.find(appt => {
    ...
    if (activeAppt) {
      const apptStart = timeToMinutes(activeAppt.time);
      const apptDuration = parseInt(activeAppt.duration) || 30;
      const apptEnd = apptStart + apptDuration;

      // Add as a booked slot
      slots.push({
      ...
      // Jump current to the end of this appointment
      current = apptEnd;
    } else {
      // Find the next upcoming appointment
      const nextAppt = sortedAppts.find(appt => timeToMinutes(appt.time) > current);
      let slotDuration = defaultDuration;

      if (nextAppt) {
        const nextStart = timeToMinutes(nextAppt.time);
        // If it overlaps the next appointment, cap it
        if (current + slotDuration > nextStart) {
          slotDuration = nextStart - current;
        }
      }
